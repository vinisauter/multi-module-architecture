 ...
 */
public class CustomFlutterEngine {
...
//    @NonNull private final FlutterEngineConnectionRegistry pluginRegistry;
    ...
    // Engine Lifecycle.
    @NonNull private final Set<io.flutter.embedding.engine.FlutterEngine.EngineLifecycleListener> engineLifecycleListeners = new HashSet<>();

    @NonNull
    private final io.flutter.embedding.engine.FlutterEngine.EngineLifecycleListener engineLifecycleListener =
            new io.flutter.embedding.engine.FlutterEngine.EngineLifecycleListener() {
                @SuppressWarnings("unused")
                public void onPreEngineRestart() {
                    Log.v(TAG, "onPreEngineRestart()");
                    for (io.flutter.embedding.engine.FlutterEngine.EngineLifecycleListener lifecycleListener : engineLifecycleListeners) {
...
//     * FlutterRenderer#startRenderingToSurface(Surface)}.
     ...
     */
    public CustomFlutterEngine(@NonNull Context context) {
        this(context, null);
    }

    /**
     * Same as {@link #CustomFlutterEngine(Context)} with added support for passing Dart VM arguments.
     *
     * <p>If the Dart VM has already started, the given arguments will have no effect.
     */
    public CustomFlutterEngine(@NonNull Context context, @Nullable String[] dartVmArgs) {
        this(context, /* flutterLoader */ null, /* flutterJNI */ null, dartVmArgs, true);
    }

    /**
     * Same as {@link #CustomFlutterEngine(Context)} with added support for passing Dart VM arguments and
     * avoiding automatic plugin registration.
     *
     * <p>If the Dart VM has already started, the given arguments will have no effect.
     */
    public CustomFlutterEngine(
    ...
    /**
     * Same as {@link #CustomFlutterEngine(Context, String[], boolean)} with added support for configuring
    ...
    /**
     * Same as {@link #CustomFlutterEngine(Context, CustomFlutterLoader, FlutterJNI, String[], boolean)} but with
    ...
    public CustomFlutterEngine(
            @NonNull Context context,
            @Nullable CustomFlutterLoader flutterLoader,
            @NonNull FlutterJNI flutterJNI) {
        this(context, flutterLoader, flutterJNI, null, true);
    }

    /**
     * Same as {@link #CustomFlutterEngine(Context, CustomFlutterLoader, FlutterJNI)}, plus Dart VM flags in
    ...
    /**
     * Same as {@link #CustomFlutterEngine(Context, CustomFlutterLoader, FlutterJNI, String[], boolean)}, plus the
    ...
    public CustomFlutterEngine(
        ...
        CustomFlutterInjector injector = CustomFlutterInjector.instance();
...
//            GeneratedPluginRegister.registerGeneratedPlugins(this);
     ...
     */
    @NonNull
    /*package*/ io.flutter.embedding.engine.FlutterEngine spawn(
            @NonNull Context context,
            @NonNull DartExecutor.DartEntrypoint dartEntrypoint,
        ...
        FlutterJNI newFlutterJNI =
                flutterJNI.spawn(
                        ...
                        initialRoute);
        return new io.flutter.embedding.engine.FlutterEngine(
                context, // Context.
    ...
    public void destroy() {
        ...
        for (io.flutter.embedding.engine.FlutterEngine.EngineLifecycleListener listener : engineLifecycleListeners) {
...
//        pluginRegistry.destroy();
     ...
     * onPreEngineStart()}.
     */
    public void addEngineLifecycleListener(@NonNull io.flutter.embedding.engine.FlutterEngine.EngineLifecycleListener listener) {
        engineLifecycleListeners.add(listener);
    }

    /**
     * Removes a {@code listener} that was previously added with {@link
     * #addEngineLifecycleListener(io.flutter.embedding.engine.FlutterEngine.EngineLifecycleListener)}.
     */
    public void removeEngineLifecycleListener(@NonNull io.flutter.embedding.engine.FlutterEngine.EngineLifecycleListener listener) {
