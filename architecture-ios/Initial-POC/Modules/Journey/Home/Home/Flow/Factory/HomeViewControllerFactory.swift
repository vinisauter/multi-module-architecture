...
public class HomeViewControllerFactory: BaseViewControllerFactory<HomeBusinessModel, HomeAnalyticsProtocol> {
    ...
    func makeHomeIndexViewController() -> HomeIndexViewController {
        return HomeIndexViewController(viewModel: HomeIndexViewModel(businessModel: businessModel, analytics: customAnalytics as? HomeIndexAnalyticsProtocol, flowDelegate: flow as? HomeIndexFlowDelegate))
    }
}
