...
public class LoginViewControllerFactory: BaseViewControllerFactory<LoginBusinessModel, LoginAnalyticsProtocol> {
    ...
    func makeLoginViewController(isIndex: Bool = false) -> LoginIndexViewController {
        let viewModel = LoginIndexViewModel(businessModel: businessModel, analytics: defaultAnalytics as? LoginIndexAnalyticsProtocol, flowDelegate: flow as? LoginIndexFlowDelegate)
    ...
    func makeForgotPasswordViewController(isIndex: Bool = false) -> ForgotPasswordViewController {
        let viewModel = ForgotPasswordViewModel(businessModel: businessModel, analytics: defaultAnalytics as? ForgotPasswordAnalyticsProtocol, flowDelegate: flow as? ForgotPasswordFlowDelegate)
        viewModel.isIndex = isIndex
